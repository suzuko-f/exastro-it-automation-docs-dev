...
| ITA Conductorにおける標準的な :index:`作業フロー` は以下の通りです。

#. | 機器情報を登録/確認する(Ansible共通)
#. | オペレーションを登録/確認する(基本コンソール)
#. | ITAの各 :index:`Driver` からMovementを登録する
...
- | Movementの登録方法については、各Driverの :index:`利用手順マニュアル` を参照して下さい。 
...
- | 「Conductor call」で実行した :index:`作業フロー` については、それぞれ個別に共有ディレクトリパスを持ちます。
  | （:index:`作業フロー` を跨いでいるMovemetについては、共有の対象外となります。）
...
.. figure:: /images/ja/conductor/condudtor_edit_and_excute/conductor_main.png
   ...
   Conductor編集/作業実行メニュー画面( :index:`編集モード` )
...
.. figure:: /images/ja/conductor/condudtor_edit_and_excute/view_mode.png
   ...
   「Conductor編集/作業実行」メニュー（:index:`「閲覧」モード`）
...
| :index:`閲覧モード` にて、 :guilabel:`作業実行` をクリックすると作業実行設定画面が表示されます。
...
.. tip:: | **オペレーションの指定について**
   ...
   | :index:`編集/更新モード` でスキップの設定は :guilabel:`登録` / :guilabel:`更新` により設定が保存されます。
